...
$q2=mysqli_query($conn,$se2) or die(mysqli_error($conn));
	  ...
	  while($d2=mysqli_fetch_array($q2))
        {    if($d2['3']==1)            //dislikes column
	            {  echo '<script language="javascript">';
                   echo 'alert("You can dislike the post only once.")';
                   echo'</script>'; 
	         ...
	         if($d2['2']==1)            //likes column
                {   if($d2['5']<2)       //counter column
				         {     $up1="update choice_once set likes='0', dislikes='1', counter='2' where category='".$cg."' and content='".$a."'  and Emailid='".$e."'";
  ...
  else { $in="insert into choice_once (category,content,likes,dislikes,Emailid,counter) values ('".$cg."','".$a."',0,1,'".$e."',1)";
         $inqu=mysqli_query($conn,$in);
		 
		 $se4="select dislikes from choice where category='".$cg."' and content='".$a."'";
         $q4=mysqli_query($conn,$se4);
         while($d4=mysqli_fetch_array($q4))
              {  
                   $count=$d4[0];  
                   $count=$count+1;
                   $up3="update choice set dislikes='$count' where category='".$cg."' and content='".$a."'";
